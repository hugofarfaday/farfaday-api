<?php class ArticleController extends DatabaseController {
    ...
    public function affectDataToRow(&$row, $sub_rows){
        ...
        if(isset($sub_rows['client'])){
            $appuser = array_filter($sub_rows['client'], function($item) use ($row) { 
        ...
        if(isset($sub_rows['categorie'])){
            $tags = array_values(array_filter($sub_rows['categorie'], function($item) use ($row) { 
                return $item->Id_article == $row->Id_article;
            }));
            if(isset($tags)){
                $row->tags_list = array_column($tags,'categorie');
            }
        }
    }
}?>